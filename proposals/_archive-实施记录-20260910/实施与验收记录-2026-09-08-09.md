...
> 本文件承载主方案文档（`../RW-Agent平台化改造总方案-v2.22.md`）在 v2.22 结构重组前附录 D 内的全部实施与验收批次记录。
...
> 同层其它归档：`proposals/_archive-四文档合并-20260908/`（v2.x 之前身文档）；回归测试遗留提案按批次归档于 `proposals/_archive-回归测试提案/`（历次 e2e 产物）。
