...
> 版本：v2.21（2026-09-10 导航定版 + 模型观测 + 装配向导 + 工具集 + 技能库 + 知识库治理 + 进化集 + 扩展中心共识：§7.2 四组 15 页（应用并入扩展中心）；§7.4-7.9 模型观测/装配向导/工具集/技能库/知识库/进化集；§7.10 扩展中心（插件·MCP·应用统一资产、四层指标、需求反馈闭环、月度巡检、需求≥3 升级阈值）；仅收录已达成共识，不含讨论过程）
