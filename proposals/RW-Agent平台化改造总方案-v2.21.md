...
> 版本：v2.21（2026-09-10 后台导航定版 + 模型观测 1.0 方案回填：§7.2 板块结构更新为四组十六页（模型/平台/应用/系统），含 Agent 装配向导/扩展中心/应用视图类型/任务中心 等已共识定版与模型观测 1.0 看板方案；仅收录已达成共识，不含讨论过程）
...
### 7.5 Agent 装配向导（专项议题，方案待定稿）
- 议题背景：新建一个 Agent(壳) 时"要给它装配什么"当前不够清晰（原壳开发页为散落表单）。
- 装配维度清单（专项细化）：①身份：key/name/description/persona/领域说明；②工具面：preset∩force_on/off∩全局启用集（壳内三态可视化）；③模型策略：默认模型/允许模型/预算；④技能：平台技能勾选 + 壳自建技能；⑤知识范围：壳私有知识库 + 是否注入全局；⑥扩展：MCP（本壳要接的外部服务）/ 插件勾选；⑦应用：本壳可用应用勾选；⑧对外形态（后置）：网页嵌入/API/上下文钩子；⑨护栏：权限/规则（allow-deny 壳级）/审批策略。
- 用户已定方向：开发好一个壳=一个独立 Agent；壳建设时勾选可用扩展与应用；"要开发插件/应用"由 intake skill 驱动问询。
