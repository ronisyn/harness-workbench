...
> 版本：v2.21（2026-09-10 导航定版 + 模型观测 1.0 + Agent 装配向导 + 工具集 UI v2/生命周期：§7.2 四组十六页；§7.4 模型观测看板；§7.5 装配向导（双模式/模板/伪配置防护）+ 壳日报→进化集审批闭环；§7.6 工具集 Tab+开关列表 UI v2 与生命周期（数据驱动淘汰、默认启用≠常用以数据为准）；仅收录已达成共识，不含讨论过程）
