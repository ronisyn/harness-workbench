...
> 版本：v2.21（2026-09-10 导航定版 + 各模块共识：§7.2 四组 15 页；§7.4 模型观测；§7.5 装配向导；§7.6 工具集；§7.7 技能库；§7.8 知识库治理；§7.9 进化集；§7.10 扩展中心；§7.11 任务/审计/设置（任务绑目标+跑一次+失败告警；审计按会话回溯+归档；设置真实性收窄清单+系统提示词用法+命中率告警首页/进化集联动）；仅收录已达成共识，不含讨论过程）
...
### 3.5 Agent（壳）模块总览（2026-09-10 汇总）
- **定义**：壳=平台内**租户级独立 Agent**（开发好一个壳=一个独立 Agent）；运行于平台（880），非独立进程；可被外部平台经"壳对外形态"调用。
- **字段集**见 §3.1（v1 冻结）；**装配**= §7.5 向导（8 步/双模式/模板/伪配置防护）；**版本管理**= §3.2 shellpacks git。
- **必要数据（最少可跑）**：key + name +（persona 或模板）+ 模型策略（默认模型或自动路由）；其余可跳用默认。
- **对外形态（后置专项）**：①网页嵌入（浮标/iframe）②会话 API（成熟项目调用）③上下文注入钩子。
- **渠道多租户（2026-09-10 确认后置）**：按壳独立渠道（壳绑自己的飞书应用/公众号/webhook）与**多部门多账号路由**（账号/群维度→路由到对应壳会话）——现网渠道=平台级单绑定（env 单飞书应用，会话 channel 仅 web/task），`channels.bindings`/`credentials` 字段存在未接线。**凭证按壳分存随连接器机制一起做**（§8 凭证规则：不进 DB 明文、只存引用），不与渠道实例化拆分。
...
### 7.2 板块结构（2026-09-10 导航定版 v2.21：4 组 15 页，去跨组连续编号）
...
- **扩展中心**（2026-09-10 并入：插件/MCP/应用 统一资产心智，见 §7.10）：插件（自研代码能力包，如 Excel 收发/PDF 编译）· MCP（外部服务连接适配，GitHub/易仓/Shopify）· **应用**（带壳身份成品入口，如 SKU 规格书助手/biz-eval）统一管理——卡片墙 + 四层指标 + 需求反馈闭环 + 月度扩展巡检；壳装配向导勾选本壳可用扩展；应用视图类型见 §7.10（chat/form 1.0，embed/page 后置）。
...
### 7.10 扩展中心（2026-09-10 共识）
- **定位与导航**：应用并入扩展中心（导航 A，用户定）——第三组变为 Agent / 扩展中心（应用市场不单列；壳内可用应用入口放 Agent 装配向导 + 壳详情）。
- **统一资产心智**：插件 / MCP / 应用 = 同一类"可装载业务资产"（平台自研→测试→上架→被壳装载→监控→升级），共享同一生命周期管理；差异只在运行时形态（插件=壳内常驻能力零件；应用=壳内点开成品入口；MCP=外部服务连接）。
- **三类形态与示例**：插件=代码级能力（Excel 收发/多表合一制表/下载、PDF 解析编译下载、视频拆解若含代码动作）；应用=带入口成品（SKU 规格书编译助手、biz-eval）；MCP=外部服务（GitHub 已接；易仓/Shopify=需开发的薄适配层）。**判定法（用户认可）**：有"离开代码做不了的硬动作"（解析/生成/IO/算法）→ 插件/应用；纯"教模型怎么做"→ 技能。
- **插件需注册**：manifest（名称/版本/能力声明/scope global 或按壳/凭证需求）；**平台只研发+测试+上架，不给总平台/其它壳自动装载**——壳需要时在装配向导勾选（用户：平台开发、壳装载才好用；总平台 RW 甚至可以不用它）。
- **页面结构（卡片墙）**：三类 Tab（插件/MCP/应用/全部）；每资产卡片=名称+说明+类型角标+状态（研发/测试/已上架）+迷你指标（近 30 天 调用/失败/需求数）；详情=能力声明/版本/变更 + **四层指标区** + 需求/升级待办流 + 装载壳数（仅信息）；顶部=全资产健康总览（故障/高失败/待升级 计数）。MCP 与连接器/凭证机制一起开发（本期只定方案）。
...
- **需求反馈闭环（用户认可三层收集+一处汇总）**：①硬信号（数据自动：失败率↑/绕路/深度浅）②软信号（对话自动捕获："要是能…就好了/能不能加…/不好用" → RW 当场轻确认"记为该插件/应用升级需求？"）③主动反馈（扩展卡片/壳内使用场景"💡 提需求"按钮 → 一句话描述+自动关联资产）→ 汇入扩展卡片「需求/升级」区 → 进化集审批台统一审（采纳→立项/升级；驳回→记录）→ **月度"扩展巡检"汇总**（进化集定时任务体系）：需求候选≥3 条或故障回升 → 卡片标"建议升级 vX.Y"（阈值 3 为用户定）。

### 7.11 系统三模块：任务 / 审计 / 设置（2026-09-10 共识）
**任务页（机制管理+执行历史）**：
- 任务列表卡：名称/cron/prompt 摘要/模型/运行状态/上次结果摘要/执行历史可展开（近 N 次 时间·结果·成本·成败）；新建任务表单（名称/cron 助手/prompt/模型/权限）；每任务卡操作：暂停/启用/**▶ 跑一次（新增，手动触发+审计）**/编辑/删除。
- **任务与目标绑定（用户定）**：每个任务都可绑进化目标（没目标设定时任务干啥）——任务页与进化集都能看绑定（任务页看"此任务绑了哪些目标"，进化集看"目标由哪些任务执行"）；#4/#3 等进化任务带"进化任务"标记，可跳进化集。
- **失败告警（用户要）**：任务失败 → 平台醒目处（首页状态带）+ 进化集审批台生成记录。
- 权限：暂不设创建限制（单账号）。
**审计页（独立于进化；用户授权按最优方案设计，看不懂先看成果再问）**：
- 过滤器：时间范围/动作类型分类（工具·知识·壳管理·任务·模型·登录）/壳/关键词；流水表：时间|账号|动作|详情（截断+悬停全文）|壳。
- **按会话回溯**：支持从某会话看它全部动作（tool_calls 轨迹与 audit 联动）；审计页与对话页互跳。
- **定期归档**：90 天前审计入归档（避免主表膨胀），归档仍可查。
- 详情默认收起、点击展开（性能+敏感最小暴露）；detail 沿用 redactSecrets 脱敏。
**设置页（纯运行时参数，用户按真实性清单收窄）**：
- 定位：只留"真接线、不重复"的平台运行参数；与工具集（access_rules/toolset_enabled）、扩展中心（mcp_servers）、模型广场（default_models）重复的一律移走；死参数删除。
- **保留清单**：温度 / 时间预算(0=不限) / 轮次上限 / 循环检测次数 / 连续失败保护 / 并发对话上限 / 假完成打回(防自欺) / 任务总预算(0=不限) / 上下文折叠组(折叠间隔/保留消息/触发字符/输入截断——降本关键) / 系统提示词(全局) / **缓存命中率目标(新增)**。
- **删除**：selfchange_budget_yuan（全库无消费=死参数）；qualityCostBias 归壳字段（设置页不出现，壳模型策略标注"预留未接线"）；max_parallel_tools 并入高级折叠组。
- **每参数给"改它会发生什么"一句话中文说明**（用户：过去看不懂、不知真假——真接线参数逐一解释，见参数真实性审计结论：第一类守护类/第二类高级保险/第三类死参数已清）。
- **系统提示词用法**：全局文本→注入每次对话（首条 system：user instructions）；分层=全局<壳 persona<项目 AGENTS.md；留空=不加全局指令；别放"只对某场景"的话（那是 persona 的活）。
- **缓存命中率目标告警呈现（用户确认）**：设置页设阈值 → **首页（工作台首屏）状态带横条**（如"命中率 6.3% < 目标 20%"）+ **进化集审批台**（同时生成建议→可转行动）；不弹窗不刷屏。
