 ...
 *   · format / formatVersion 必须认识（见文件头 ③）；
...
export async function importConversation(obj, { dryRun = true, pool = defaultPool } = {}) {
  const stats = validateSessionExport(obj); // 任何校验失败都在写库之前抛出
  ...
  try {
    ...
    const [ins] = await conn.query(insertSql('conversations', convCols, 1), convCols.map((c) => bind(c, obj.conversation[c], false)));
    ...
    for (const part of PARTS) {
      const src = part.key === 'usage' ? obj.usage.rows : obj[part.key];
