...
export const TOOL_MANIFEST = {
  ...
  fetch_spill: { tier: 'core', cn: '取回溢出结果', when: '上下文提示"全文已存 <路径>"时，按范围取回被溢出的工具结果全文', not: '读取类工具溢出的定位符是源文件路径，用 read_file_range 直接读', ex: 'fetch_spill {path:"<溢出文件绝对路径>", offset:0, length:20000}', exempt: true },
  ...
  read_file: { tier: 'core', cn: '读取文件', when: '读文本内容、改前先读、查实现细节', not: '大文件超限用 read_file_range；列目录用 list_dir', ex: 'read_file {path:"<文件绝对路径>"}', defaultOn: true, light: true },
  ...
  repo_map: { tier: 'core', cn: '代码地图', when: '大仓库/陌生目录任务开始时先取结构地图（目录树+行数+imports+符号）', not: '小目录直接 list_dir；找符号位置用 grep_search', ex: 'repo_map {dir:"<目录绝对路径>"}', defaultOn: true, light: true },
  ...
  run_command: { tier: 'expert', cn: '执行命令', when: '无专门工具覆盖的系统操作（npm/部署/服务重启）', not: 'cat/ls/grep/sed/head/tail/find/cd/echo 用专门工具', ex: 'run_command {command:"npm install"}', defaultOn: true },
