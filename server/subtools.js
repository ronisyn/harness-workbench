...
import { MANIFEST_NAMES } from './tools/registry.js';
...
export function subtoolRefusal(whitelist, name) {
  ...
  if (whitelist.has(String(name))) return null;
  return '工具 ' + name + ' 不在本子代理的工具清单内（派发时用 tools 收窄过）。请改用清单内的工具完成任务；若确实需要该工具，把需求回报给父代理。';
}
