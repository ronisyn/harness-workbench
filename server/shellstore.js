...
export async function importShell(pack) {
  ...
  if (existing) {
    await db.query(
      `UPDATE shells SET name=?, description=?, persona=?, domain_text=?, model_policy=?, tools_preset=?, tools_force_on=?, tools_force_off=?, knowledge_scopes=?, skills_allow=?, guardrails=?, channels=?, ui_brand=?, eval_ref=?, intent_rules=?, updated_at=NOW() WHERE id=?`,
      [row.name, row.description, row.persona, row.domain_text, row.model_policy, row.tools_preset,
        ...
        row.channels, row.ui_brand, row.eval_ref, row.intent_rules, existing.id]
    );
  ...
  const r = await db.query(
    `INSERT INTO shells (skey, name, description, persona, domain_text, model_policy, tools_preset, tools_force_on, tools_force_off, knowledge_scopes, skills_allow, guardrails, channels, ui_brand, eval_ref, intent_rules, status, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())`,
    [row.skey, row.name, row.description, row.persona, row.domain_text, row.model_policy, row.tools_preset,
      row.tools_force_on, row.tools_force_off, row.knowledge_scopes, row.skills_allow, row.guardrails,
      row.channels, row.ui_brand, row.eval_ref, row.intent_rules, 'enabled']
  );
