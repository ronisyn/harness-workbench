...
# 附：场景与首页信息架构 v1（2026-09-03 用户逐项确认）
> 本附件的原则：**页面由场景决定，场景由"你打开时的目的"决定**。界面融合的 A/B/C 模型暂不讨论，先定"登录后看到什么"。

## S0 第 0 屏 = 飞书提示卡（真正的第一眼）
- 推送节奏（已确认 A）：**早晨汇总一次 + 关键事件即时推**（出现待复测/待确认/资源支持/任务卡住即推）。
- 卡片内容（按优先级）：
  1. 需要你处理：待复测 N（附 项目#编号 列表）/ 待确认 N / 资源支持 N
  2. 昨夜/当前执行一句话：完成 X / 卡住 Y（原因）
  3. 成本一句话 + 明细入口
- 动作：`[处理待办]`（深链落到网页队列第一个待办）、`[看项目进展]`（深链到首页项目脉搏）。

## S1 第 1 屏 = 网页首页：L+P 融合（队列 + 项目脉搏）
- 左栏=决策队列（处理为主）：三 tab（待复测/待确认/资源支持），逐个内嵌处理流
  - 待复测项内嵌：任务工单摘要 + AI 执行报告入口 + 验收项 → 过/打回（fails 留痕，对齐 hello /api/mine）
  - 待确认项：需求要点 + 确认/调整
  - 资源支持：缺什么、怎么给
- 右栏=项目脉搏（扫视为主，**默认展开 3 个活跃项目，其余折叠可展开**，已确认）：
  每项目一行摘要：进度环/进行中与待复测数/卡住标记/成本；细节点项目→项目工作台。
- 底部常驻对话输入（不打断处理，随时丢想法/立项）。
- 统计大卡/知识库/会话历史等全部退到导航，平时不出现。

## S2 第 2 层 = 按需（出问题才进）
- 需要单独讨论的需求 → 打开该需求文档详读 + 围绕它和 AI 讨论调整
- 出状况的项目 → 进项目工作台
- **衍生缺口（新待办）**：对话需支持"围绕某个需求/工单的讨论线程"（绑定需求 ID，讨论沉淀回 requirements.disc）——现状对话是会话级。

## 数据接口要求（首页/卡片成形后各端供给）
- 队列 ← hello「我的任务」mine（待复测/待确认/资源支持）
- 项目脉搏 ← hello projects + parseWorkflow 状态 + RW 驱动器状态与账本成本
- 对话/立项/执行流 ← RW（讨论人格 + 契约驱动器 + usage 账本）
- 早晨汇总 ← 聚合层（RW 调度，把三 tab + 昨夜执行 + 成本压成一张卡，经飞书卡片/微信推送）
