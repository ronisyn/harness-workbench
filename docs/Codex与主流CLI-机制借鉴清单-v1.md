...
> 📌 历史施工蓝图（2026-09-10 治理标注）：本文档=内核进化期（2026-09-05~08）的借鉴落地记录。**编号撞名警示**：本文 P1–P3 与蓝图 A《平台开发全集清单》P1–P26 **同仓库撞号且含义互不相干**（本文 P=借鉴批 hooks/checkpoint/repo_map/MCP；蓝图 P=内核决策定稿）——阅读勿混，本文已不再维护。批次已全部落地（✅，代码在位）；唯一 ⬜ 项「多模型交叉验证（延至 2026-10 再评估）」决策已登记入总方案 B §11.4 后置。落地状态以代码与总方案 B 为准；`server/tools/checkpoint.js` 头注引用本文 §2（代码注释级历史出处）。
...
4. **⬜ 多模型交叉验证（可选）**（主会话 2026-09-08 拍板：延后至 2026-10 再评估，见 §4 标注）
...
5. **✅ MCP 生态接入（Codex/Claude Code 支持 MCP 工具服务器）**（2026-09-06 落地，状态见 docs/MCP接入状态.md）
   - 落地：server/mcp.js + MCP 管理面板（配置存 settings 表 mcp_servers；入口历经迁移：对话页⚙设置抽屉 2026-09-09 退役 → 后台→系统→1.8 设置 → 2026-09-11 A8 批（`39a3d8486`）迁至「后台 → 应用 → 扩展中心 → MCP 外部服务接入」，以 MCP接入指南.md 为准）；github MCP server 已接入 26 个工具，
...
4. ⬜ P2-4 双模型交叉验证（可选，配合审计需求）——**主会话 2026-09-08 拍板：09 月无审计需求，延后至 2026-10 再评估；期间不再每日复读（本行 ⬜ 不代表待办，仅状态标记）**
