...
- **L1 立杆（本方案，2026-09-04 执行完成 ✅）**：度量仪表（WS0）+ 工具契约分级（WS1）+ 运行时快照（WS2）+ 准则文件（WS3）+ 设置 schema（WS4）+ 记忆分层（WS5）+ 技能与自改纪律（WS6）+ 信任契约（WS7）+ 外部模式收口（WS8）+ 验证件（WS9）。执行记录见附录C。
- **L2 自主进阶**：自我升级循环跑顺（打回率下降+沉淀增长持续 N 周）→ 训练轮（预算/自改范围约束）按证据逐级放开。
- **L3 优秀 Web 智能体**：新领域、长任务、自我优化照常成立——准则仍在，高跷全拆。

### 0.3 L1→L2 观察期安排（2026-09-04 起）
- **节奏**：周日 9:00 定时任务「KPI周报」（scheduled_tasks，cron `0 9 * * 0`）跑 `scripts/kpi.mjs --days 7`，贴五 KPI+工具榜 top8+2 行解读（与上周对比）；基线=baseline-2026-09-04.json。
- **进入 L2 的证据（全部满足才放训练轮）**：连续 4 周 KPI1 打回率 ≤15% 且周成本不增；KPI4 沉淀每周 ≥1 条且复用 ≥3 次；KPI5 事故 0。满足后：预算/自改范围训练轮按周放宽一档。
- **回落处理**：任一周 KPI5 事故 >0 或打回率 >40% → 暂停放宽，触发打回复盘回溯（逐条找根因→回流附录A）。
- **季度自审**（约 2026-12）：18 维模板重跑 + §1.5 外部借鉴复审（观察项 X4/X5 取证评估）+ 高跷闸门复审 + 工具健康度榜裁剪。
...
- 实现清单：
  ...
  4. 成本预算：**总账上限** `task_budget_total`（默认 ¥100/会话 24h，跨继续累计，含子代理；超限停止并提示调大，0=不限）；单段节奏提醒 `task_budget_yuan` **默认 0=关**（用户明确不需要中途暂停打扰；需要时自行开启）。
