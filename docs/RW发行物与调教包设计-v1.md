...
> 📌 历史设计（预壳时代,2026-09-10 治理修订）：主发行模型"每工作台独立实例（独立进程/独立 DB）"已被《RW-Agent 平台化改造总方案》(B) **D1=单核心多逻辑壳取代**——独立实例仅作 B §11.1 instance-pack 升级路径；§7 hello 试点以 B D2/§11.4 为准。核心概念（三物关系/差异位/回流）已并入 B §11.1/§11.4；本文件 §3/§5 运维细则（模板升级/实例 kpi 对比/git proxy/抽查单）仍为参考（B §11.4 承接细则）。
> ⚠️ 两处"以脚本为准"注：①2.2 表"平台设计蓝图应用到实例 docs/"与实际不符——`scripts/apply-pack.mjs` RULE_DOCS **不复制**蓝图 A、且含已失效的 `RW撑竿跳方案.md`（existsSync 静默跳过）→ 以脚本为准；②"壳"在此指实例领域层,与 B §2"Agent(壳)=租户级 Agent 容器"**同词异义**,勿混读。文中计数(62 工具/默认 25(+4)/技能 ×5)为该时期快照,现网=63 注册/默认 28（meta DEFAULT_TOOLSET）/技能 6（rw-dev-audit 为服务器侧,发行包 5）。
