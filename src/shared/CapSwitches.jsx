...
const DEF_GROUP_NAMES = { A: '渲染能力', B: '工具能力', C: '平台能力' };

export default function CapSwitches({ onToast, groupNames, showGroupTitle = true, chips = false }) {
  ...
  const toggleCap = async (key, v) => {
    ...
    try { await api.setCapabilities({ [key]: v }); setErr(''); }
    catch (e) {
      setCaps((c) => c.map((x) => (x.key === key ? { ...x, enabled: !v } : x)));
      const msg = '开关保存失败：' + (e.message || e);
      if (onToast) onToast(msg); else setErr(msg); // P3-1：成功清空旧 err
    }
  };
  return (
    <div>
      ...
      {['A', 'B', 'C'].map((g) => (
        <div key={g} className="rw-cap-group">
          <div className="rw-cap-gtitle">{groupNames ? groupNames[g] : (DEF_GROUP_NAMES[g] || ('组 ' + g))}</div>
          <div className={chips ? 'rw-console-toolbar' : undefined}>
